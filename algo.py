class Algo:
    ...
    def solve_foward_checking_cols(self, col=0, stars_placed=0, start_row=0):
        """Backtracking en parcourant les colonnes"""
        # Vérifier si la solution actuelle est complète et valide
        if self.is_solution_valid():
            return True
    ...
    def solve(self, choice):
        ...
        if (choice == 1):
            # On commence avec la première colonne (0)
            if self.solve_backtracking_cols(col=0):
                ...
                return False
        if (choice == 2):
            if self.solve_foward_checking_cols(col=0):
                print("Solution trouvée et affichée")
                return True
            else:
                print("Aucune solution trouvée")
